...
'''class DataGenerator(Dataset):
	...
	def __init__(self, root="Data/", augment=True, debug_mode=False, scale=None, split=None):
	...
	def load_img_and_anno(self, index):
		...
		if self.scale: #scale down if self.scale != None
		...
		for member in root.findall('object'): #Iterate over each bounding box
			...
			if self.scale: #scale down if self.scale != None
				xmin, ymin, xmax, ymax = [x//self.scale for x in [xmin, ymin, xmax, ymax]]

			x_cent = int((xmin+xmax)/2) #Find the x,y coordinates of the centre
			y_cent = int((ymin+ymax)/2)
		...
		if self.scale: #scale down if self.scale != None
			img = cv2.resize(img, (0,0), fx=1/self.scale, fy=1/self.scale)			
		if self.augment:
